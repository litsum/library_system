...
bool LibraryDatabase::loadData() {
    ...
    while (bookQuery.next()) {
        ...
        statusQuery.prepare("SELECT COUNT(*) FROM borrowed_books WHERE book_id = :isbn");
        statusQuery.bindValue(":isbn", QString::fromStdString(isbn));
        if (statusQuery.exec() && statusQuery.next()) {
            available = (statusQuery.value(0).toInt() == 0);
        }
        ...
        Book book(name, author, publisher, isbn, type);
        book.setAvailable(available);  // 使用books向量

        books.emplace_back(book);  // 使用emplace_back正确添加
    }
    if (!db.commit()) {
...
bool LibraryDatabase::saveData()  {
    ...
    if (!deleteBooksQuery.exec("DELETE FROM books")) {
        qDebug() << "Failed to delete books:" << deleteBooksQuery.lastError().text();
        db.rollback();
        return false;
    }
    // 使用事务批量插入
    db.transaction(); // 开始新的事务
    ...
    for (const auto& reader : readers) {
        ...
        for (size_t i = 0; i < borrowedBooks.size(); ++i) {
            ...
            std::string bookId = "";
            // 修改：先删除借书记录关联
            QSqlQuery deleteBookRelations(db);
            if (!deleteBookRelations.exec("DELETE FROM borrowed_books")) {
                qDebug() << "Failed to delete book relations:" << deleteBookRelations.lastError().text();
                db.rollback();
                return false;
            }
